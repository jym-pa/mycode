...
def devicereboot(devicecmd): # devicecmd==dict
    ...
    for ip in devicecmd.keys(): # looping through the dict
        print(f'{crayons.red("REBOOTING NOW")}. .. ... on {ip}') # fstring with RED
        # we'll learn to write code that connects to devices here
        for mycmds in devicecmd[ip]:
            print(f'Attempting to sending command --> "shutdown"')
            # we'll learn to write code that sends cmds to device here
    return None
...
#    ## deprecating in favor of external file reference
...
#    ["interface eth1/1", "shutdown"], "10.3.0.1":["interface eth1/5", "no shutdown"]}
    ...
    # PWD needs to be the same dir this file is located in
    with open("devicecmd.json", "r") as devicecmdfile:
        devicecmd = json.load(devicecmdfile) # decode JSON from the file to pythonic data
    ...
    devicereboot(devicecmd) # call function to reboot all devices
